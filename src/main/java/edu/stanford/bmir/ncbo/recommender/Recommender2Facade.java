...
public class Recommender2Facade {
	 ...
	 *         recommended to annotate the input provided
	 */
	// private static List<RecommendationResultTO> getCombinedRanking_old(
	// String input, int inputType, int maxElementsInCombination,
	// List<String> ontologyUris, double wc, double prefScore,
	// double synScore, double multiTermScore, double wd, double ws,
	// double wa, double waPv, double waUmls, double waPm) {
	// List<RecommendationResultTO> results = new
	// ArrayList<RecommendationResultTO>();
	// CoverageEvaluator coverageEvaluator = new CoverageEvaluator();
	// AcceptanceEvaluator acceptanceEvaluator = new AcceptanceEvaluator();
	// HashMap<String, List<AnnotationTO>> annotations = AnnotatorAdapter
	// .getAnnotations(input, inputType, ontologyUris);
	//
	// // Only the ontologies that have annotations will be evaluated
	// List<String> selectedUris = new ArrayList<String>(annotations.keySet());
	//
	// System.out.println("SelectedUris: " + selectedUris.size());
	// // In order to improve performance, only a subset of the ontologies are
	// // selected to enter the evaluation stage of ontology combinations
	// List<String> selectedUrisForCombinedRanking = RecommenderUtil
	// .selectUrisForCombinedRanking_old(annotations);
	//
	// System.out.println("SelectedUrisForCombinedRanking: "
	// + selectedUrisForCombinedRanking.size());
	//
	// // Calculates all the combinations of ontologyUris without repetition
	// ArrayList<ArrayList<String>> combinedUris = CombinatoricalOperations
	// .getCombinations(selectedUrisForCombinedRanking,
	// maxElementsInCombination);
	// System.out.println("Number of combinations: " + combinedUris.size());
	//
	// AnnotatorUtil util = new AnnotatorUtil();
	//
	// // /*** pruebas marcos begins ***/
	// // ArrayList<ArrayList<String>> combinedUris0 = new
	// ArrayList<ArrayList<String>>();
	// // combinedUris0.addAll(combinedUris);
	// // combinedUris.clear();
	// // List<Double> annotationsScores = new ArrayList<Double>();
	// // for (ArrayList<String> uris : combinedUris0) {
	// // int sum = 0;
	// // List<AnnotationTO> allAnnotations = new ArrayList<AnnotationTO>();
	// // for (String uri : uris) {
	// // allAnnotations.addAll(annotations.get(uri));
	// // }
	// // List<AnnotationTO> bestAnnotations = util.getBestAnnotationsForText(
	// // input, allAnnotations, prefScore, synScore, multiTermScore);
	// // /*double totalScore =
	// // util.getTotalScoreForAnnotations(bestAnnotations, prefScore, synScore,
	// multiTermScore);
	// // annotationsScores.add(totalScore);
	// // if (totalScore >= 400)
	// // combinedUris.add(uris);*/
	// // }
	// // /*Collections.sort(annotationsScores);
	// // System.out.println(annotationsScores);*/
	// //
	// // System.out.println("Number of combinations: " + combinedUris.size());
	// //
	// // /*** pruebas marcos ends ***/
	...
	// long startTimeCov = System.currentTimeMillis();
	// // All the annotations are put together
	// List<AnnotationTO> annotationsForCombination = new
	// ArrayList<AnnotationTO>();
	// // For each uri in each group
	// for (String uri : uris) {
	// annotationsForCombination.addAll(annotations.get(uri));
	// }
	...
	// // the rest of the scores proportionally
	// double totalScore = combinedCoverageResult.getScore();
	// HashMap<String, Double> individualCoverageScores = new HashMap<String,
	// Double>();
	// for (String uri : uris) {
	// double individualScore = 0;
	// for (AnnotationTO annotation : combinedCoverageResult
	...
	// double combinedNormSpecScore = 0;
	// for (String uri : uris) {
	// SpecializationResultTO specializationResult = SpecializationEvaluator
	// .evaluate(input, inputType, uri);
	// combinedSpecScore += (individualCoverageScores.get(uri) / totalScore)
	// * specializationResult.getScore();
	// combinedNormSpecScore += (individualCoverageScores.get(uri) / totalScore)
	// * specializationResult.getNormalizedScore();
	// }
	// SpecializationResultTO combinedSpecializationResult = new
	// SpecializationResultTO(
	// combinedSpecScore, combinedNormSpecScore);
	// // System.out.println(combinedCoverageResult);
	// // System.out.println(combinedSpecializationResult);
	// long timeSpec = System.currentTimeMillis() - startTimeSpec;
	// totalTimeSpec += timeSpec;
	//
	//
	...
	// double combinedUmlsScore = 0;
	// for (String uri : uris) {
	// AcceptanceResultTO acceptanceResult = acceptanceEvaluator
	// .evaluate(uri, waPv, waUmls, waPm);
	// combinedAccepScore += (individualCoverageScores.get(uri) / totalScore)
	// * acceptanceResult.getScore();
	// combinedPubMedScore += (individualCoverageScores.get(uri) / totalScore)
	// * acceptanceResult.getPubMedScore();
	...
	// // System.out.println("*** Execution times for iteration ***");
	// // System.out.println("Coverage evaluation: " + timeCov + " ms");
	// // System.out.println("Specialization evaluation: " + timeSpec + " ms");
	// // System.out.println("Acceptance evaluation: " + timeAcc + " ms");
	//
	// // TODO: arreglar esto, importante
	// /*** Combined Detail of Knowledge evaluation ***/
	...
	//
	// RecommendationResultTO result = new RecommendationResultTO(uris,
	// ontologyUrisBioPortal, ontologyNames, aggregatedScore,
	// combinedCoverageResult, combinedSpecializationResult,
	// combinedAcceptanceResult, combinedDetailResult);
	//
	// results.add(result);
	//
	// }
	// long evaluationTime = System.currentTimeMillis() - startEvaluationTime;
	// long avgTimeCov = totalTimeCov / combinedUris.size();
	// long avgTimeSpec = totalTimeSpec / combinedUris.size();
	// long avgTimeAcc = totalTimeAcc / combinedUris.size();
	//
	// System.out.println("*** Total execution times ***");
	// System.out.println("Evaluation time: " + evaluationTime + " ms");
	// System.out.println("Average coverage evaluation: " + avgTimeCov + " ms");
	// System.out.println("Average specialization evaluation: " + avgTimeSpec +
	// " ms");
	// System.out.println("Average acceptance evaluation: " + avgTimeAcc +
	// " ms");
	//
	// Collections.sort(results);
	// return results;
